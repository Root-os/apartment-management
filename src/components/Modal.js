    ...
    <div className="fixed inset-0 flex justify-center items-center bg-gray-500 bg-opacity-50 z-50">
      <div className={`bg-white rounded-lg shadow-lg max-w-sm w-full p-6 border-2 ${modalStyles}`}>
        <div className="flex justify-between items-center">
          <h2 className="font-semibold text-xl">{type === 'success' ? 'Success' : 'Error'}</h2>
        ...
        <div className="mt-6 flex justify-center">
          <button 
            onClick={onClose} 
            className={`px-4 py-2 rounded-lg ${type === 'success' ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-red-600 text-white hover:bg-red-700'}`}
          >
            Close
          </button>
        </div>
