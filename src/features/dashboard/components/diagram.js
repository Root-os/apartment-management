...
const UnitStatusReport = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data?.unitsReport) {
          const { occupiedUnits, availableUnits, underMaintenanceUnits } = response.data.unitsReport;
          setData({
  ...
  useEffect(() => {
    ...
    const dataset = [
      { label: `Occupied: ${data.totalOccupied}`, value: data.totalOccupied > 0 ? data.totalOccupied : 0.001, color: "red" },
      { label: `Available: ${data.totalAvailable}`, value: data.totalAvailable > 0 ? data.totalAvailable : 0.001, color: "green" },
      { label: `Maintenance: ${data.totalUnderMaintenance}`, value: data.totalUnderMaintenance > 0 ? data.totalUnderMaintenance : 0.001, color: "blue" },
    ];
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row items-center justify-center w-full max-w-5xl gap-10 py-10 px-6 mx-auto">
        ...
        <svg
          id="legend"
          className="w-full md:w-1/3 h-auto max-h-[500px]"
        ></svg>
      </div>
    </div>
  );
