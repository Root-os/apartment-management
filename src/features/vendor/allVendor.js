...
const VendorsPage = () => {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await axios.delete(`${process.env.REACT_APP_BASE_URL}vendors/${selectedVendor.id}`);
  ...
  const columns = [
    { key: 'name', label: 'Full Name', render: (row) => `${row.fname} ${row.lname}` },
    { key: 'phone', label: 'Phone' },
    { key: 'email', label: 'Email' },
    { key: 'address', label: 'Address' },
    { key: 'ServiceType.name', label: 'Service Type', render: (row) => row.ServiceType?.name },
    {
        label: 'Actions',
        key: 'actions',
        render: (row) => (
        ...
        <div className="mt-10 fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-base-100 p-6 rounded-md w-full max-w-lg mx-4 overflow-y-auto" style={{ maxHeight: '80vh' }}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleEdit(); }}>
              <div className="mb-4">
                <label htmlFor="fname" className="block text-sm font-medium text-white-700">
                  First Name
                </label>
                <input
                  type="text"
              ...
              <div className="mb-4">
                <label htmlFor="lname" className="block text-sm font-medium text-white-700">
                  Last Name
                </label>
                <input
                  type="text"
              ...
              <div className="mb-4">
                <label htmlFor="serviceTypeId" className="block text-sm font-medium text-white-700">
                  Service Type
                </label>
                <select
                  id="serviceTypeId"
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-base-100 p-6 rounded-lg w-full max-w-md mx-4">
            <h2 className="text-xl mb-4">Are you sure you want to delete this vendor?</h2>
